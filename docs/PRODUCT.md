...
The Library follows the `UVKcL` Pencil composition: a compact top bar with centered search, a desktop filter rail, a prominent blue `.forge` import panel, and a dense responsive tool grid. Import validation is explicit and the panel states that nothing runs during import. Status and category filters collapse into selects on smaller screens. Categories are derived from actual bundled and installed manifests rather than a predefined taxonomy. A tool has one to three categories, normally one or two; Forge receives the current category list, reuses an accurate existing name first, and creates a short new name only when needed. Legacy installed manifests with a single `category` are normalized in memory.

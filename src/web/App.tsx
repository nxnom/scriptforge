    ...
    <aside className="sidebar">
      ...
      <nav className="nav">
        ...
        <a className="nav-item active" href="#library">
          <Library size={16} /> Library <span>4</span>
        </a>
        <a className="nav-item" href="#queue">
          <Sparkles size={16} /> Forge queue
        </a>
        <span className="nav-heading section-gap">Filters</span>
        <a className="nav-item" href="#all">
          <Wrench size={16} /> All tools
        </a>
        <a className="nav-item" href="#ready">
          <Check size={16} /> Ready <span>1</span>
        </a>
        <a className="nav-item" href="#planned">
          <Plus size={16} /> Planned <span>3</span>
        </a>
      </nav>
...
function ToolCard({
  ...
  tool: { id: string; name: string; description: string; category: string; icon: string; status: string };
}) {
    ...
    <article className={`tool-card ${ready ? "" : "planned"}`}>
      <div className="card-top">
        ...
        <span className={`status ${ready ? "ready" : "soon"}`}>
          {ready ? <Check size={12} /> : <Sparkles size={12} />}
          {ready ? "Ready" : "Planned"}
        </span>
      </div>
      ...
      <div className="card-footer">
        ...
        <button type="button" disabled={!ready}>
          {ready ? "Open" : "Soon"} <ArrowRight size={13} />
        </button>
      </div>
    </article>
  );
    ...
    <div className="app-shell">
      ...
      <main className="main">
        <header className="page-header">
          <div>
            <h1>Library</h1>
            <p>Small, focused tools forged for your workflow.</p>
          </div>
        ...
        <section className="empty-panel">
          ...
          <div className="empty-copy">
            ...
            <p>
              Describe the next utility you need. Codex will forge it locally and nothing runs before you approve it.
            </p>
          </div>
          <Button variant="outlined" size="sm">
            <Hammer size={15} /> Forge your first tool
          </Button>
        </section>

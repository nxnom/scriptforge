...
- The iframe intentionally blocks CDN scripts, remote modules, external stylesheets, remote fonts, fetch, and every other runtime network request. Never use a script src, remote import, CDN URL, or runtime npm install. Prefer native HTML, SVG, Canvas, and browser APIs for compact charts and visualizations. If a small third-party browser library is genuinely worthwhile, verify a pinned package version and its redistribution license during Forge, then inline only its required browser distribution into ui.html while preserving its license notice. Keep the complete ui.html below the 3 MB candidate review limit. If that is not practical, implement the focused UI with native browser APIs instead. Test the final offline iframe rather than only testing the library in Node.

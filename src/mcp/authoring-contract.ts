...
- On wide canvases around 800px and above, do not leave the primary input and result cards at content height above a large blank lower canvas. Make the page shell a min-height: 100vh flex column, let the main grid or workspace flex: 1 with align-items: stretch, and stretch its primary cards through the available row height. Give long result collections an internal scroll region when needed. At the narrow breakpoint, return the main layout and cards to height: auto so the stacked tester fits its content instead of forcing every card to viewport height.
- In the narrow stacked layout, keep inputs, choices, actions, progress, and status grouped from the top with ordinary compact gaps. Never use justify-content: space-between, space-around, or space-evenly on a full-height vertical form, input card, or control column; those rules create awkward empty bands between related controls. Use flex-start for controls and let only a genuine result region grow when useful.
...
- If the interface says users can drag and drop files, implement dragenter, dragover, dragleave, and drop handlers rather than only styling a drop zone. Prevent the browser default and stop propagation for the drag/drop flow, set a visible active state, read event.dataTransfer.files on drop, apply the same count, type, and size validation as the file input, and pass accepted files through the normal selection path. A claimed drop target that does not accept a real dropped File is a broken primary input.
- When a tool works on existing folders by absolute path, its custom folder browser must be a compact accessible dialog that matches the tool theme. Include Close, Up, an editable current-path field with Go, a scrollable folder list, an optional Show hidden folders control, and a clear Add this folder or Choose this folder action. Center labels and icons inside their controls and keep the dialog usable in the narrow Forge tester.
...
- Before presenting the candidate, inspect the CSS as if the iframe were 480px wide and again at 1200px. At 480px nothing may clip or scroll horizontally and stacked cards should remain content-height. At 1200px the workspace must use the canvas intentionally: primary cards should stretch through the available height instead of ending above a large blank lower area, and the page must not leave a narrow centered island or unused space.
...
When the interface advertises drag-and-drop, handle dragenter, dragover, dragleave, and drop on the visible target. Call preventDefault and stopPropagation, update the active visual state, take files from event.dataTransfer.files, validate them exactly like files chosen through the input, and feed them into the same selected-file state. Do not add “drop” copy or styling without functional drop handling. During the UI check, dispatch or manually perform a real file drop and verify that the selected-file preview and primary action update.

Do not add arbitrary file-size, total-upload, page-count, media-duration, or image-pixel limits. ScriptForge runs locally so practical capacity is determined by the user's machine. Keep type and operation validation, and clearly report unavoidable limits imposed by an underlying executable or file format.

For a tool whose main input is one or more existing filesystem folders, never ship only a text input or textarea. Provide both manual absolute-path entry and an Add folder or Choose folder button that opens a custom in-tool folder browser. Implement browsing through the same run bridge with files: [] and a read-only input action such as { action: "browseFolders", path, showHidden }. run.mjs should use Node os and path APIs to default to the user's home directory, resolve roots and parents cross-platform, validate that the target exists and is a directory, and return data such as { currentPath, parentPath, folders: [{ name, path }] }. The dialog must support Close, Up, editing the current path and pressing Go, opening listed folders, optionally showing hidden folders, and confirming the current folder. For multiple-folder tools, confirmation appends the path as a removable row instead of replacing all selections. Keep direct path paste/edit as a fallback and show browse or validation errors inside the dialog. Do not use input type=file with webkitdirectory as the only folder picker: browsers expose copied files and relative names, not the absolute directory path needed for in-place work. Folder browsing itself must never perform the tool's destructive or mutating action; that still requires the separate primary action and any appropriate preview or confirmation.

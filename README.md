...
The bundled image tools accept a local PNG, JPEG, or WebP file, so no separate sample dataset is required. The image resizer and App Icon Exporter are powered by `sharp` and require no system executable.

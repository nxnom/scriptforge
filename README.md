...
- **Tool identity:** Grid cards, list rows, and detail sidebars hash normalized primary-category text into a stable OKLCH icon color, so each category keeps the same identity everywhere without a predefined color registry. Cards, category chips, and hover states remain neutral to avoid visual noise; attention-only status badges and the quiet indigo Built-in trust badge remain semantic. Ready tools need no redundant badge; exceptions stay visible.
